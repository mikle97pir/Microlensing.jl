...
function par_evaluate_mass_homotopy(masses, positions, E, Λ, δs, rate, find_root)
    mass_homotopy = Microlensing.create_mass_homotopy(masses, positions, E, Λ)
    init1, init2 = Microlensing.find_corrections(masses, positions, δs, E, Λ)
    start_roots = Microlensing.find_start_roots(mass_homotopy, positions, init1, init2, δs, find_root)
    hom(start) = Microlensing.homotopize(start, δs, 1., mass_homotopy, rate, mass_lim_func, find_root)
